...
def greedy_decode(model, inp, mode="categorical", temperature=1.0, k=None, token_count=None):
    """
    ...
    Args:
        ...
        token_count (int): number of tokens to generate; default: None, i.e., generate until the model predicts an end
    ...
    try:
        with torch.no_grad():
            while True:
                ...
                if (prediction == end_token) or (token_count is not None and count == token_count):
                    return inp.squeeze()
...
def generate(model_, inp, save_path="./bloop.mid", mode="categorical", temperature=1.0, k=None,
    ...
    start = time.time()
    token_ids = greedy_decode(model=model_, inp=inp, mode=mode, temperature=temperature, k=k, token_count=token_count)
    end = time.time()
